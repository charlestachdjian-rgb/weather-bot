...
Also: SUM_ANOMALY for pricing deviations.
...
async def notify_telegram(session: aiohttp.ClientSession, message: str) -> None:
    """Mock telegram function for paper trading - just logs to console"""
    logger.info("TELEGRAM MOCK: %s", message[:100])
...
def detect_signals(markets: list[dict],
    ...
    Layer 5: Guarded Ceiling NO / Locked-In YES (5 safeguards)
    Plus: SUM_ANOMALY
    """
    signals = []
    ...
    for m in open_markets:
        ...
        # ── Layer 5: GUARANTEED_NO_CEIL (guarded) ───────────────────────────
        ...
        # ── Layer 5: LOCKED_IN_YES (guarded) ────────────────────────────────
        if lo is not None and hi is not None and int(hour_local) >= LOCK_IN_HOUR:
            if lo - ROUNDING_BUFFER <= daily_high <= hi + ROUNDING_BUFFER:
                if yes < 0.80:
                    blocked, reasons = should_block_risky_signal(
                        hour_local, daily_high, lo,
                        metar_history, synop_history, om_hourly)
                    if not blocked:
                        signals.append({
                            ...
                            "no_price":    no,
                            "entry_price": yes,
                            "edge":        round(1.0 - yes, 3),
                            "note":        (f"daily_high={daily_high}°C → {label}, "
                                           f"hour={int(hour_local)} CET, YES={yes:.0%} "
                                           f"[ALL 5 GUARDS PASSED]"),
                            "daily_high":  daily_high,
                            "token_id":    m.get("token_id", ""),
                        })
                    else:
                        logger.info("LOCKED_YES BLOCKED on %s: %s", label, "; ".join(reasons))

    # ── SUM_ANOMALY ─────────────────────────────────────────────────────────
    if open_markets:
        deviation = yes_sum - 1.0
        if abs(deviation) > SUM_TOL:
            if deviation > 0:
                worst = max(open_markets, key=lambda x: x["yes_price"])
                signals.append({
                    "type":        "SUM_OVERPRICED",
                    "tier":        0,
                    "our_side":    "NO",
                    "range":       range_label(*worst["temp_range"]),
                    "yes_price":   worst["yes_price"],
                    "no_price":    worst["no_price"] or (1.0 - worst["yes_price"]),
                    "entry_price": worst["no_price"] or (1.0 - worst["yes_price"]),
                    "edge":        round(deviation, 3),
                    "note":        f"YES sum={yes_sum:.3f} (+{deviation:.3f} over 1.0). Buy NO on most overpriced: {range_label(*worst['temp_range'])} @ {worst['yes_price']:.3f}",
                    "daily_high":  daily_high,
                    "token_id":    worst.get("no_token_id", ""),
                })
            else:
                def _proximity_score(mkt: dict) -> float:
                    lo_m, hi_m = mkt["temp_range"]
                    mid = ((lo_m or 0) + (hi_m or 0)) / 2 if (lo_m and hi_m) else (lo_m or hi_m or 0)
                    return -abs(mid - daily_high)

                best = max(open_markets, key=_proximity_score)
                signals.append({
                    "type":        "SUM_UNDERPRICED",
                    "tier":        0,
                    "our_side":    "YES",
                    "range":       range_label(*best["temp_range"]),
                    "yes_price":   best["yes_price"],
                    "no_price":    best["no_price"] or (1.0 - best["yes_price"]),
                    "entry_price": best["yes_price"],
                    "edge":        round(-deviation, 3),
                    "note":        f"YES sum={yes_sum:.3f} ({deviation:.3f} under 1.0). Buy YES on closest bracket to daily_high: {range_label(*best['temp_range'])} @ {best['yes_price']:.3f}",
                    "daily_high":  daily_high,
